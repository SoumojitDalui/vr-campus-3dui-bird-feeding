...
public class MetaXRSimulatorEnabler : MonoBehaviour
{
	...
	private static string GetSimulatorJsonPath()
	{
		...
		{
			string candidate = Path.Combine(rootPath, "MetaXRSimulator", "SIMULATOR.json");
			if (File.Exists(candidate)) return candidate;
		}

		string pluginsRoot = Path.Combine(Application.dataPath, "Oculus", "VR", "Plugins");
		if (Directory.Exists(pluginsRoot))
		{
			foreach (string versionDir in Directory.GetDirectories(pluginsRoot))
			{
				string candidate = Path.Combine(versionDir, "MetaXRSimulator", "SIMULATOR.json");
				if (File.Exists(candidate)) return candidate;
			}
		}

		return !string.IsNullOrEmpty(rootPath) ? Path.Combine(rootPath, "MetaXRSimulator", "SIMULATOR.json") : null;
	}
	...
	private static string GetSimulatorDllPath()
	{
		...
		if (!string.IsNullOrEmpty(rootPath))
		{
			string candidate = Path.Combine(rootPath, "MetaXRSimulator", "SIMULATOR.dll");
			if (File.Exists(candidate)) return candidate;
		}
		...
		if (Directory.Exists(pluginsRoot))
		{
			foreach (string versionDir in Directory.GetDirectories(pluginsRoot))
			{
				string candidate = Path.Combine(versionDir, "MetaXRSimulator", "SIMULATOR.dll");
				if (File.Exists(candidate)) return candidate;
			}
		}

		return !string.IsNullOrEmpty(rootPath) ? Path.Combine(rootPath, "MetaXRSimulator", "SIMULATOR.dll") : null;
	}
